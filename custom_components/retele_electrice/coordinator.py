"""DataUpdateCoordinator for Retele Electrice.
...
class ReteleElectriceCoordinator(DataUpdateCoordinator):
    """Coordinate data fetching from the Retele Electrice API."""
    ...
    async def _import_statistics(self, records: list[dict[str, Any]]) -> None:
        ...
        if import_stats:
            self._push_statistics(
                ...
                name=f"Retele Electrice {self.pod} Import",
                stats=import_stats,
            )
        if export_stats:
            self._push_statistics(
                statistic_id=export_id,
                name=f"Retele Electrice {self.pod} Export",
                stats=export_stats,
            )

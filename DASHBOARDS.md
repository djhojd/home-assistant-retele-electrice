...
# Recommended dashboards for Retele Electrice
...
Replace `RO005EXXXXXXXXX` (uppercase, in titles) and `ro005exxxxxxxxx` (lowercase, in entity IDs) with your POD.
    ...
    sections:
      - type: grid
        ...
        cards:
          ...
          - type: tile
            entity: sensor.retele_electrice_ro005exxxxxxxxx_last_sync
            name: Ultima sincronizare
            icon: mdi:sync
          - type: tile
            entity: button.retele_electrice_ro005exxxxxxxxx_sync_data
            name: Sincronizare manuala
            icon: mdi:cloud-sync
          - type: statistic
            entity: retele_electrice:ro005exxxxxxxxx_import
            ...
            period:
              calendar:
                period: month
          - type: statistic
            entity: retele_electrice:ro005exxxxxxxxx_export
...
Replace `RO005EXXXXXXXXX` / `ro005exxxxxxxxx` with your POD.
...
views:
  - type: sections
    ...
    sections:
      - type: grid
        title: RO005EXXXXXXXXX — Period totals
        cards:
          - type: heading
...
- **POD ID substitution.** The placeholder `RO005EXXXXXXXXX` (uppercase, in titles) and `ro005exxxxxxxxx` (lowercase, in entity IDs) appears throughout each snippet. Find/replace both forms with your own POD.
...
- **Diacritics.** Snippets deliberately use plain ASCII for editor portability. Add Romanian diacritics back in the `name:` / `title:` fields if your editor handles UTF-8 cleanly.
